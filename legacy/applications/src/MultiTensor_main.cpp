...
po::variables_map parse_command_line(int ac, char **av)
{
    ...
    desc.add_options()("help", "produce help message")("folder,f", po::value(&folder)->default_value(""), "set folder name")("adj,a", po::value(&adj)->default_value("adjacency.dat"), "set adjacency file name")("end_file,E", po::value(&end_file)->default_value(".dat"), "set end of file name")("w_file,w", po::value(&w_file)->default_value("w.dat"), "set initialization file for w")("L,l", po::value(&L)->default_value(4), "set number of layers")("intialization,i", po::value(&initialization)->default_value(0), "set intialization flag")("K,k", po::value(&K)->default_value(5), "set number of groups")("N_real,r", po::value(&N_real)->default_value(1), "set number of realizations")("maxit,t", po::value(&maxit)->default_value(500), "set maximum number of iterations")("tolerance,e", po::value(&tolerance)->default_value(0.1), "set convergence tolerance")("err,g", po::value(&err)->default_value(0.1), "set perturbation in w")("output,o", po::value(&out_adjacency)->default_value(false), "flag to output single-layer adjacencies")("assortative,A", po::value(&assortative)->default_value(false), "flag to restrict to assortative model")("rseed,z", po::value<unsigned>(), "sets random number seed")("decision,y", po::value<int>(&decision)->default_value(10), "program converges after this # repeats of the decision variables");
...
int main(int ac, char **av)
{
    ...
    if (out_adjacency)
        out_graph(folder, A); // uncomment if you want to output the graph
