...
void randomize_w(vector<vector<vector<double>>> &w)
{
    ...
    for (int i = 0; i < L; i++)
        for (int k = 0; k < K; k++)
            ...
            {
                if (q == k)
                    w[k][q][i] = real01(); // for better convergence
                else
                    w[k][q][i] = w[q][k][i] = real01();
            }
}
...
void initialize_w(vector<vector<vector<double>>> &w, istream &file)
{
    ...
    for (int a = 0; a < L; a++)
        for (int k = 0; k < K; k++)
            for (int q = 0; q < K; q++)
                w[k][q][a] += err * real01(); // add some noise so to have non-zero entries out of the diagonal
}
...
void initialize_v(vector<vector<double>> &v, istream &file)
{
    ...
    for (string line; getline(file, line);)
    {
        ...
        if (a >= 0)
        {
            ...
            {
                is >> v[a][k];
                max_entry = std::max(max_entry, v[a][k]);
            } // need to know magnitude of max_entry to add noise consequently
        }
...
void initialize(int initialization, vector<int> u_list, vector<int> v_list, vector<vector<double>> &u, vector<vector<double>> &v, vector<vector<vector<double>>> &w, string file, string w_file, vector<Graph> A)
{
    ...
    { // everything is initialized
        ...
        for (int i = 0; i < u_to_be_removed.size(); i++)
            for (int k = 0; k < K; k++)
                u[u_to_be_removed[i]][k] = 0.;
        for (int i = 0; i < v_to_be_removed.size(); i++)
            for (int k = 0; k < K; k++)
                v[v_to_be_removed[i]][k] = 0.;
    }
    else if (initialization == 2)
    { // only w is initialized
        std::ifstream in_w;
...
int idx(string const &id, vector<Graph> &g)
{
    ...
    if (mit == idx_map.end())
    { // Case where the node has not been already added into the graph
        for (int a = 0; a < g.size(); a++)
        { // Run through layers
            if (a == 0)
                idx_map[id] = add_vertex(VertexProperties(id), g[a]); // Update the map just once
            else
                add_vertex(VertexProperties(id), g[a]);
        }
        return idx_map[id];
    }
    return mit->second;
...
int read_graph(string folder, string adj, vector<Graph> &A)
{
    ...
    {
        if (tok == "E")
        {                // control variable "E"; to check that the input file containes the edge information
            ...
            for (int alpha = 0; alpha < L; alpha++)
            {
                ...
                if (is_edge > 0)
                    for (int weigth = 0; weigth < is_edge; weigth++)
                        add_edge(vertex(v1, A[alpha]), vertex(v2, A[alpha]), A[alpha]);
            ...
            for (int j = 0; j < N; j++)
            {
                ...
                for (int k = 0; k < K; k++)
                    for (int q = 0; q < K; q++)
                    {
                        ...
                        if (edge(i, j, A[a]).second)
                            log_arg += uvw; // if edge exists, consider this term inside the log argument
...
void update_old_variables(vector<vector<double>> &u_old, vector<vector<double>> &v_old, vector<vector<double>> u, vector<vector<double>> v, vector<vector<vector<double>>> &w_old, vector<vector<vector<double>>> w, vector<int> u_list, vector<int> v_list)
{
    ...
    for (int i = 0; i < v_length; i++)
        for (int k = 0; k < K; k++)
            v_old[v_list[i]][k] = v[v_list[i]][k];
    for (int i = 0; i < u_length; i++)
        for (int k = 0; k < K; k++)
            u_old[u_list[i]][k] = u[u_list[i]][k];
    for (int k = 0; k < K; k++)
        for (int q = 0; q < K; q++)
            for (int a = 0; a < L; a++)
                w_old[k][q][a] = w[k][q][a];
...
void check_for_convergence(vector<Graph> A, int &it, double &l2, double tolerance, int &coincide, int decision, bool &convergence, vector<vector<double>> u, vector<vector<double>> v, vector<vector<vector<double>>> w)
{
    ...
    if (it % 10 == 0)
    {
        ...
        if (abs(l2 - old_L) < tolerance)
            coincide++; // can use maxD or abs(l2-old_L) to check convergence
        else
            coincide = 0;
...
void output_results(std::vector<Graph> A, string file, string end_file, int N_real, double maxL, vector<vector<double>> u_f, vector<vector<double>> v_f, vector<vector<vector<double>>> w_f)
{
    ...
    for (int i = 0; i < N; i++)
    {                                // output node following their int index order
